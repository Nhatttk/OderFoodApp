...
class AdminType : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_admin_type)

        supportActionBar?.setTitle("Admin")
        supportActionBar?.setDisplayHomeAsUpEnabled(true)

//        addFragment(AddTypeFragment())
//        replaceFragment(MainFragment())
    ...
    private fun getFragmentItem() {
        ...
        smoothbtBar.setOnItemSelectedListener {
            when (it.itemId) {
                R.id.addFragment -> replaceFragment(AddTypeFragment())
                R.id.fetchingFragment -> replaceFragment(FetchingTypeFragment())
